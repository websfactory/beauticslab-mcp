...
**결정 P3**: Workers Rate Limiting 바인딩으로 `/register` 에 **IP당 20건/60초**. 실사용과 두 자릿수 배 차이라 정상 클라이언트는 닿지 않는다.
...
**검증** (`wrangler dev --local`): 같은 IP 25연타 → 1~20번 201, 21~25번 429. 다른 IP 는 201 정상. 회귀 확인 — AS 메타데이터·PRM·server-card 200, 인증 없는 `/mcp` 401, 없는 경로 404, `authorize` 302 모두 이상 없음.
